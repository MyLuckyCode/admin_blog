...
class PictureModel extends Model{
    ...
    protected $_tables=array(DB_FREFIX.'picture_title');
    ...
    public function addPicture(){   //添加相册，后台 ajax 用
    ...
    public function editPicture(){  //修改相册，后台 ajax 用
        $_where=array("name='{$this->_R['name']}'");
        if($this->isOne($_where)){
            echo '{"state":"repeat","info":"该相册已存在"}';
            exit;
    ...
    public function deletePicture(){
        ...
        $count = $_pictureItem->movePicrureInitial($oldId);    //删除分组前，先将分组组内的图片移动到未分组  
    ...
    public function getPictureList(){   //获取全部相册 后台 ajax 用,后台模板用
        return parent::select(array('id','name','count','operation'),null);
    }
    
    public function getPictureCount($id){  //统计图片总数 
        if($id==0){    //查找全部相册
            $_where=array("id!=0");
        }else {
            $_where=array("id=$id");

...
class WorksModel extends Model{
    private $_fields=array('id','title','target','info','imgUrl','disabled');
    protected $_tables=array(DB_FREFIX.'works');
    ...
    public function addWorks(){     //有用,后台ajax调用
    ...
    public function findOne(){  
        ...
        return parent::select(array('id','title','target','imgUrl','info','disabled'),
                                array('where'=>$_where,'limit'=>'1'));
    }
    
    public function findAllDisabled(){ //有用 前台和后台的ajax调用
        return parent::select(array('id','title','target','info','imgUrl'),array('where'=>array('disabled=1')));
    }
    
    public function findAll($pageCurrent,$page_size){ //有用 后台模版用
        $_start=($pageCurrent-1)*$page_size;
        $_end=$page_size;
        $all = parent::select(array('id','title','target','imgUrl','disabled','info','date','date smallDate'),
